...
Simple Fun #321: Scratch lottery II
...
function scratch(lottery){
  return lottery.reduce((acc, val) => acc + counting(val), 0);
}

function counting(characterSet){
  let money = characterSet.split(' ').at(-1);
  let unique = new Set(characterSet.split(' ').slice(0,3));
  
  if(unique.size === 1 || (unique.size === 2 && unique.has('###'))){
    return money === '###' ? 10000 : +money; 
  }
