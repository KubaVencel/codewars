...
function verifyLatinSquare(array, m) {
  let n = array.length
  if(array.some(x => x.length != n)) return "Array not square"
  if(n != m)                         return "Array is wrong size"
  
  for(let i = 0; i < n; i++){
    let rows = new Set(), cols = new Set()
    for(let j = 0; j < n; j++){
      if(rows.has(array[i][j])) return `${array[i][j]} occurs more than once in row ${i + 1}`
      rows.add(array[i][j])
      if(cols.has(array[j][i])) return `${array[j][i]} occurs more than once in column ${i + 1}`
      cols.add(array[j][i])
