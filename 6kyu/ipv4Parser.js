...
function ipv4Parser(ipv4Address, subnetMask) {
  const ipv4Octets = ipv4Address.split`.`, subnetOctets = subnetMask.split`.`;
  return [
    ipv4Octets.map((octet, i) => octet &  subnetOctets[i]).join`.`,
    ipv4Octets.map((octet, i) => octet & ~subnetOctets[i]).join`.`
  ];
}
