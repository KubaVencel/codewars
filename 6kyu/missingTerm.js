...
function missingTerm(n, littleChouSum) {
  const start = (n * n - n) / 2;
  const lastNum = Math.ceil((Math.sqrt(8 * (littleChouSum + start) + 1) - 1) / 2);
  
  return (lastNum * lastNum + lastNum) / 2 - littleChouSum - start || lastNum + 1;
}
