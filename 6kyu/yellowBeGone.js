...
const yellowBeGone = colorNameOrCode => {
    const vocab = {
      gold: "ForestGreen",
      khaki: "LimeGreen",
      lemonchiffon: "PaleGreen",
    ...
    // if colorName
    if (colorNameOrCode.toLowerCase() in vocab) return vocab[colorNameOrCode.toLowerCase()];
  
    // if colorCode
    if (colorNameOrCode.startsWith('#')) {
      const RGB = {
        R: colorNameOrCode.slice(1, 3),
        G: colorNameOrCode.slice(3, 5),
        B: colorNameOrCode.slice(5, 7),
      };
      
      const dec2hex = num => parseInt(num, 16);
      const hex2dec = hex => ('00' + hex.toString(16)).slice(-2);
      
      const R = dec2hex(RGB['R']);
      const G = dec2hex(RGB['G']);
      const B = dec2hex(RGB['B']);
      
      if ( R > B && G > B) {
        let isCaseSensitive = colorNameOrCode.toUpperCase() === colorNameOrCode;
        const sortedHexArr = [R, G, B].sort((a, b) => a - b);
        const swappedColorCode = '#' + hex2dec(sortedHexArr[0]) + hex2dec(sortedHexArr[2]) + hex2dec(sortedHexArr[1]);
      
        return isCaseSensitive ? swappedColorCode.toUpperCase() : swappedColorCode;           
      }
    }
