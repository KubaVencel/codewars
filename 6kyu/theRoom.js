...
function dramameter(room) {  
  const numPeople = room.length  
  let result = {}
  
  for(let player = 0; player < numPeople; player++){
    
    const [playerName, playerPartner, playerFriends, playerLoves] = room[player]
    
    result[playerName] = playerPartner ? playerLoves.length : 0
           
    for (let i = 0; i < numPeople; i++) {     
      let partnersName = room[i][0]
      let partnersFriends = room[i][2]
      
      partnersFriends.forEach((partnersFriend) => {
        if (playerPartner?.includes(partnersName) && playerLoves.includes(partnersFriend)) {
          result[playerName]++
        }
      })     
    }   
    ...
    for (let j = 0; j < numPeople; j++) {  
      const [friendsName, friendsPartner] = room[j]
        
      playerFriends.forEach((friend) => {
        if (friendsName.includes(friend) && playerLoves.includes(friendsPartner)){
          result[playerName]++
        }
      })
    }
    
    result[playerName] += (!playerPartner && playerLoves.length == 0) ? 1 : 0
  }
  
  let sumPoint = () => Object.values(result).reduce((sum, curent) => sum + curent)
  
  if (sumPoint(result) == 0 && result.Tommy !== undefined){
    result.Tommy = 5
  }   
  return result 
}
