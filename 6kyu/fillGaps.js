...
function timesheetHelper(i, previousValue, timesheet) {
	if (timesheet.length > i) {
		if (timesheet[i] == null) {
			var result = timesheetHelper(i + 1, previousValue, timesheet)
			if (result == previousValue)
				timesheet[i] = result;
			return result;
		}
		timesheetHelper(i + 1, timesheet[i], timesheet)
		return timesheet[i];
	}
	return timesheet[i - 1];
}
