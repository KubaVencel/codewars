...
function noMusical(start_class, end_class, musical_performed_every, duration_of_enrolment_in_school) {
  let noMusicalCount = 0;

  // Loop through each starting class year
  for (let year = start_class; year <= end_class; year++) {
    let musicalYearFound = false;

    // Check each year the class would be in school
    for (let i = 0; i < duration_of_enrolment_in_school; i++) {
      // If there's a musical in any of those years, mark it
      if ((year + i - start_class) % musical_performed_every === 0) {
        musicalYearFound = true;
        break;
      }
    }

    // If no musical was found during any of the class years, increment the count
    if (!musicalYearFound) {
      noMusicalCount++;
    }
  }

  return noMusicalCount;
}
