...
function anagrams(subjects, memories, players) {
	...
	const assign = ['Alice', 'Bob', 'Carol', 'Dan'];
	const teams = { Alice: 0, Bob: 0, Carol: 1, Dan: 1 };
	let [player1, player2] = [assign[players[0]], assign[players[1]]];

	const tmp = order.map(x => x.join(''));
	const idx = tmp.indexOf(`${player1}${player2}`);;
	...
	const findWords = (subject, player, turn) => {
		const list = memories[assign.indexOf(player)];
		for (const word of list) {
			if (word.split('').sort().join('') === subject.split('').sort().join('')) {
				if (turn === 0) {
					if (word === subject) score1++;
					else {
						score1 += 2;
						found.push(word);
					}
	...
	for (const subject of subjects) {
		score1 = 0, score2 = 0, found = [];
		findWords(subject, player1, 0);
		findWords(subject, player2, 1);
		if (score1 > score2) score[teams[player1]]++;
		else if (score2 > score1) score[teams[player2]]++;
		order.push(order.shift());
		[player1, player2] = order[0];
	}

	[score1, score2] = score;
	return score1 > score2 ? 0 : score2 > score1 ? 1 : -1;
}
