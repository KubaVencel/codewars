...
Simple Fun #196: Counterfeit Dollar
...
const counterfeitDollar = results => {
	const good = [], bad = [];

	results.sort(a => -a.endsWith('even')).forEach(weighing => {
		const [left, right, res] = weighing.match(/\w+/g);

		if (res === 'even') return good.push(...left, ...right);

		[left, right].map(coins => coins.replace(new RegExp(`[${good.join``}]`, 'g'), ''))
			.forEach((coins, idx) => bad.push(...[...coins].map(coin => [coin, idx ^ res === 'up' ? 'heavy' : 'light'])));
	});

	return bad.length !== 1 ? '???' : `${bad[0][0]} is the counterfeit coin and it is ${bad[0][1]}.`;
}
