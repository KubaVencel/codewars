...
function powRootPandigit(val, n, k) {
	let res = []
	for (let i = Math.floor(val ** (1 / n) + 1e-12) + 1; res.length < k; i++) {
		if (isPandigital(i)) {
			let a = i ** n
			if (a > 987654321) { break }
			if (isPandigital(a)) { res.push([i, a]) }
		}
	}
