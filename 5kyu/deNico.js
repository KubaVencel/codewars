...
const deNico = (key, m) => {
	const orderedKey = key.split('').sort((a, b) => a.localeCompare(b));
	const keyCodes = key.split('').map(l => orderedKey.indexOf(l) + 1);

	const splits = [];
	const messageLetters = m.split('');
	while (messageLetters.length > 0) {
		splits.push(messageLetters.splice(0, key.length))
	}

	return splits.map(s => keyCodes.map(i => s[i - 1]).join('')).join('').trimRight();
}
