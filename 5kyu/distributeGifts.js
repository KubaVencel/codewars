...
function distributeGifts(n, wishes, seq = []) {
	if (n <= 0) return !n ? seq : "Mission Failed!"

	for (let i = 0; i < wishes.length; i++) {
		let poped = wishes.splice(i, 1),
			get = distributeGifts(n - poped, wishes, seq.concat(poped))
		if (get != "Mission Failed!") return get
		wishes.splice(i, 0, poped[0])
	}
