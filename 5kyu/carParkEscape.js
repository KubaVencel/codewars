...
function escape(carpark) {
	const length = carpark[0].length - 1;
	const myfloor = carpark.findIndex((floor) => floor.includes(2))

	return structuredClone(carpark.slice(myfloor)).reverse().map((floor, i, reversed) => {
		const me = floor.indexOf(2)
		const stair = floor.indexOf(1)
		...
		if (stair > -1) {
			return reversed[i + 1].indexOf(1) - stair
		}
		return reversed[i + 1].indexOf(1) - length
	}).reduce((moves, dist, i) => {
		if (dist < 0) moves.push("R" + Math.abs(dist))
		if (dist > 0) moves.push("L" + Math.abs(dist))
		if (dist !== 0) moves.push("D1")
		if (dist === 0 && !moves.slice(-1)[0]?.startsWith("D")) moves.push("D0")
		if (dist === 0 && moves.slice(-1)[0]?.startsWith("D")) moves.push(moves.pop().replace(/\d+/, num => Number(num) + 1))
		return moves
	}, []).slice(0, -1).reverse()
}
