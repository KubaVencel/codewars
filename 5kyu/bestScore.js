...
function bestScore(teamA, teamB) {
	const add = (a, b) => a + b;
	const totalScore = (team, line) => team.map(x => x < line ? 2 : 3).reduce(add, 0);
	const difference = (line) => totalScore(teamA, line) - totalScore(teamB, line);

	const diffs = teamA.concat(teamB, Infinity).map(difference);
